...
class Particle {
  ...
  update() {
    // If particle is settled, don't update position and don't decrease lifespan
    if (this.settled) {
      return;
    }
  ...
  checkSettling() {
    ...
    // Settled particles never disappear
    if (this.settled) {
      return false;
    }
    // Only remove particles that are still in motion and have expired
  ...
  show() {
    ...
    if (this.settled) {
      // Use full opacity for settled particles
      fill(150, 0, 0);
      noStroke();
