...
function setup() {
  ...
  synth = new Tone.PolySynth(Tone.Synth, {
    ...
    envelope: { attack: 0.05, decay: 0.2, sustain: 0., release: 1.5 }
  }).toDestination();
...
function applyChanceNotes(originalCircle) {
  if (random() < 0.5) { // Increased chance of triggering extra notes
    let randomCircle = random(circles);
    if (!randomCircle.active) {
      randomCircle.active = true;
      synth.triggerAttack(randomCircle.chord, undefined, random(0.6, 1.2));
